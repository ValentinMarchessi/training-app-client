...
export default function Checkout() {
    ...
    const { myProduct, owner } = location.state;
    ...
    useEffect(() => {
        const makeRequest = async () => {
            try {
                ...
                if (res.status == 200) {
                    await postCreateTransaction(dispatch, myProduct.id, user.userId, data, user.accessToken);
    ...
    const stripeOptions = {
        name: 'Training App',
        image: Logo,
        description: `Your total is $${(myProduct.price + 0.4).toFixed(2)}`,
        amount: (myProduct.price + 0.4) * 100,
        token: onToken,
        stripeKey: KEY,
    };
            ...
            <div className={styles.details}>
                <div className={styles.header}>
                    ...
                    <div id={styles.merit}>
                        ...
                        <p id={styles.rating}>{myProduct.rating || '?'}/5</p>
                        <p id={styles.reviews}>({myProduct.reviews || '?'} reseñas)</p>
                    </div>
                </div>

                <div id={styles.routineCard}>
                    <div id={styles.user}>
                        <Avatar style={{ height: "50px" }} src={owner[0].profile_img} />
                        <div className={styles.info}>
                            <p id={styles.name}>{owner[0].username}</p>
                            <p id={styles.title}>{owner[0].title}</p>
                        </div>
                    </div>
