...
export default function DietDetail() {
	...
	const id = location.state.diet_id;
	const user = useSelector((state) => state.user.currentUser);
	const diet = useSelector((state) => state.diets.dietDetails);
	const imgRecipDefault = "https://t1.rg.ltmcdn.com/es/posts/4/3/8/chanfainita_20834_600.jpg";
	useEffect(() => {
		getDietDetails(dispatch, id, user.accessToken);
	...
	const dietClick = (e, day, i) => {
		if (dayView[1] === day) setDayView([!dayView[0], day, i]);
	...
	const recetDetail = (obj) => {
		...
		if (obj === null)
			return [
				<p className={style.noRecipe}>
					You have no a {option} in the day {dayView[1]}
				</p>,
				<img
					className={style.noRecipeImg}
		...
		for (const key in obj) {
			if (key === "image")
				info.push(
					<div key={i} className={style.imgRecipe}>
						<img key={i} src={obj[key] || imgRecipDefault} />
					</div>
				);
			else if (key === "title") info.push(<h3 key={i}>{obj[key][0].toUpperCase() + obj[key].slice(1)}</h3>);
			else if (key === "description" || key === "updatedAt" || key === "UserId" || key === "createdAt") continue;
			else
				info.push(
	...
	const days = () => {
		let days = [];
		diet.plain.forEach(({ day }, i) =>
			days.push(
				<span
					key={i}
					onClick={(e) => dietClick(e, day, i)}
					className={`${style.day} ${dayView[0] && dayView[1] === day && style.daySelect}`}>
